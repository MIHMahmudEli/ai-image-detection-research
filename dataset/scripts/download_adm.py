...
BASE_URL = 'https://huggingface.co/datasets/bitmind/GenImage_ADM/resolve/main/data'
DL_DIR = Path('genimage_zips/hf_shards/ADM')
DL_DIR.mkdir(parents=True, exist_ok=True)

url = 'https://huggingface.co/api/datasets/bitmind/GenImage_ADM'
r = requests.get(url)
data = r.json()
siblings = data.get('siblings', [])
shards = sorted([s['rfilename'] for s in siblings if s['rfilename'].endswith('.parquet')])
print(f'Downloading {len(shards)} ADM shards...')
