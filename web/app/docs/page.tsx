...
export default function DocsPage() {
  return (
    <div className="min-h-screen bg-cream-50 dark:bg-slate-950">
      <header className="sticky top-0 z-40 border-b border-stone-200/80 bg-cream-50/80 backdrop-blur-md dark:border-slate-800/80 dark:bg-slate-950/80">
        <div className="mx-auto flex h-16 max-w-5xl items-center gap-3 px-4">
          <a
            href="/"
            className="flex items-center gap-1.5 text-sm text-stone-500 transition-colors hover:text-blue-600 dark:text-slate-400 dark:hover:text-blue-400"
          >
            <ArrowLeft className="h-4 w-4" />
            Home
          </a>
          <span className="h-5 w-px bg-stone-200 dark:bg-slate-800" />
      ...
      <div className="mx-auto max-w-5xl px-4 py-12">
        <h1 className="mb-4 text-4xl font-bold text-stone-900 dark:text-white">
          API Reference
        </h1>
        <p className="mb-8 max-w-2xl text-stone-500 dark:text-slate-400">
        ...
        <div className="mb-12 grid gap-6 md:grid-cols-3">
          {[
            ...
            <div
              key={item.title}
              className="rounded-xl border border-stone-200 bg-cream-100 p-5 transition-all hover:-translate-y-0.5 hover:border-blue-300 hover:shadow-md dark:border-slate-800 dark:bg-slate-900 dark:hover:border-blue-700"
            >
        ...
        <div className="space-y-8">
          {endpoints.map((ep) => (
            <div
              key={ep.path}
              className="overflow-hidden rounded-xl border border-stone-200 dark:border-slate-800"
            >
              <div className="flex items-center gap-3 border-b border-stone-200 bg-cream-100 p-5 dark:border-slate-800 dark:bg-slate-900">
                <span
                ...
                <code className="font-mono text-sm text-stone-900 dark:text-slate-100">
                  {ep.path}
                </code>
              </div>
              <div className="space-y-4 bg-cream-50 p-5 dark:bg-slate-950">
                <p className="text-sm text-stone-600 dark:text-slate-300">
                  {ep.desc}
                </p>
                <div>
                  <p className="mb-1 text-xs font-medium text-stone-500 dark:text-slate-400">
                    Example Request
                  </p>
